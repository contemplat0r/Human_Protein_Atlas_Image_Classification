...
def rle_encode(img):
    ...
    Returns run length as string formatteds 
    '''
    pixels = img.flatten()
...
def rle_decode(mask_rle, shape=(768, 768)):
    '''
    mask_rle: run_length as string formated (start length)
    shape: (height, width) of array to return
    Returns numpy array 1 - mask, 0 - baskground
    '''
    s = mask_rle.split()
...
def mask_as_image(in_mask_list):
...
valid_df['counts'] = valid_df.apply(
    lambda c_row: c_row['counts'] if isinstance(c_row.EncodedPixels, str) else 0,
    1
)
...
train_df.hist(bins=15)


# In[13]:


valid_df.hist(bins=15)
...
class ShipDataset(Dataset):
    def __init__(self, in_df, transform=None, mode='train'):
        ...
        self.images_mask = [m['EncodedPixels'].values for _, m in grp]
    ...
    def __getitem__(self, idx):
        ...
        img = imread(rgb_path)
        mask = mask_as_image(self.images_mask[idx])
...
def clip(img, dtype, maxvall):
    return np.clip(img, 0, maxvall).astype(dtype)
...
def OneOf():
    def __init_(self, transforms, prob=0.5):
        self.transforms = transforms
        self.prob = prob
...
def get_jaccard(y_true, y_pred):
    epsilon = 1e-15
    intersection = (y_true * y_pred).sum(dim=-2).sum(dim=-1).sum(dim=-1)
    union = y_true.sum(dim=-2).sum(dim=-1).sum(dim=-1) + y_pred.sum(-2).sum(-1).sum(-1)
    ...
    for epoch in range(epoch, n_epochs + 1):
        ...
        try:
            ...
            for i, (inputs, targets) in enumerate(tl):
                ...
                tq.set_postfix(loss='{:.5}'.format(mean_loss))
...
def make_loader(in_df, batch_size, shuffle=False, transform=None):
    return DataLoader(
        dataset=ShipDataset(in_df, transform),
        shuffle=shuffle,
        num_workers=0,
...
# train inputs.shape: torch.Size([16, 3, 256, 256]), targets.shape torch.Size([16, 1, 256, 256])
...
out_pred_rows = []
for batch_num, (inputs, paths) in enumerate(tqdm(loader, desk='Predict')):
    inputs = variable(inputs, volatile=True)
    outputs = model(inputs)
    for i, image_name in enumerate(patchs):
        mask = F.sigmoid(outputs[i, 0]).data.cpu().numpy()
        cur_seg = binary_opening(mask>0.5, disk(2))
        cur_less = multi_rle_encode(cur_seg)
        if len(cur_less) > 0:
            for c_rle in cur_less:
